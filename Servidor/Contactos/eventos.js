...
form.addEventListener('submit', async e => {
  e.preventDefault();
  e.preventDefault()
  const responseJSON =  await fetch("http://localhost:3004/contactos", {
        method: "POST", 
        headers:{
...
list.addEventListener('click',  async e => {
  ...
  console.log(id);
    await fetch(`http://localhost:3004/contactos/${id}`, {
        method: "DELETE", 
    });
...
const getTodos = async () =>{
  const response = await fetch("http://localhost:3004/contactos", {method: "GET"});
